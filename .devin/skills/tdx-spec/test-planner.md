...
**Investigation** (first section of spec — adaptive):
- `## Investigation` — đặt ở đầu file, trước Happy Path/Reproduction Steps.
- **Task Analysis** (bắt buộc): tóm tắt task yêu cầu gì, behavior mong đợi, public interface liên quan.
- **Impact Scope** (bắt buộc): files/modules affected, downstream callers, regression risk areas.
- **Dependencies** (optional): chỉ include khi task có dependency rõ ràng (external API, shared module, data migration).
- **Risk** (optional): chỉ include khi có risk không tầm thường (performance, security, data loss).
- ht: adaptive — không ép đủ 4 section nếu task đơn giản. 2 bắt buộc + 2 tùy chọn.
